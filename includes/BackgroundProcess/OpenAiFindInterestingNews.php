...
class OpenAiFindInterestingNews extends BackgroundProcessBase {
	 ...
	 * @return void
	 */
	public static function add_to_sync( array $news_ids, array $sync_settings = [] ) {
	...
	protected function task( $item ) {
		...
		if ( empty( $sync_options ) ) {
			$sync_options = $item['sync_options'] ?? [];
		}
		$new_ids = isset( $item['ids'] ) && is_array( $item['ids'] ) ? array_map( 'intval', $item['ids'] ) : [];
		if ( count( $new_ids ) ) {
			...
			foreach ( $response->get_suggested_news_ids() as $id ) {
			...
			foreach ( $new_ids as $new_id ) {
				if ( in_array( $new_id, $response->get_suggested_news_ids(), true ) ) {
					continue;
				}
