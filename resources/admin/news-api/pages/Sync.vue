<template>
  ...
  <div>
    <ShaplaTabs alignment="center">
      ...
      <ShaplaTab name="News Keywords">
        <div class="max-w-lg mx-auto">
          <ShaplaButton theme="primary" size="large" fullwidth @click="syncNewsKeyword">Sync Now</ShaplaButton>
        </div>
      </ShaplaTab>
    </ShaplaTabs>
...
const syncNewsKeyword = () => {
  Dialog.confirm('Are you sure to sync now?').then(confirmed => {
    if (confirmed) {
      Spinner.show();
      http
          .post('admin/keywords/sync')
          .then(() => {
            Notify.success('A background task is running to sync news with OpenAI.', 'Success!')
          })
          .finally(() => {
